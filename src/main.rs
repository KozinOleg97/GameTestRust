...
fn main() {
    App::new()
        .add_plugins((DefaultPlugins, HexRendererPlugin, performance_overlay::PerformanceOverlayPlugin))
...
fn setup(mut commands: Commands) {
    ...
    let generator = ProceduralWorldGenerator::new(100, 100, 12345);
...
fn render_world(
    ...
    for (i, hex) in hex_map.get_all_hexes().iter().enumerate() {
        ...
        // Check if the hex is inside the camera's view frustum
        if camera.world_to_viewport(camera_transform, world_pos).is_ok() {
            // Hex is visible, render it
        } else {
            // Hex is outside the camera's view frustum, skip
            continue;
        }
    ...
    *has_rendered = true;
    println!("Finished rendering {} hexes (culled to {})", hex_map.size(), rendered_count);
}
